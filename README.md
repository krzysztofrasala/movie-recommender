...
CountVectorizer — Bag of Words (5,000 features, English stop words)
        │
        ▼
Cosine Similarity Matrix — N × N (float32 for compact storage)
        │
        ▼
...
The `.pkl` model files are excluded from git (too large). Rebuild them locally:
```bash
TMDB_API_KEY="your_api_key_here" python fetch_dataset.py
```
This fetches up to 4,000 popular movies from TMDB and writes `movie_dict.pkl`, `similarity.pkl`, and `movies.csv` to the project root. Takes ~5–10 minutes depending on your connection.
...
├── fetch_dataset.py            # Rebuilds movie_dict.pkl + similarity.pkl from TMDB
...
TMDB_API_KEY = "your_api_key_here"
```
4. The `.pkl` files must be committed via Git LFS or generated in a startup script
